...
func (m *collectorMetrics) snapshot() map[string]any {
    ...
    for k, v := range m.byCat { byCat[k] = v }
    for k, v := range m.bySev { bySev[k] = v }
        ...
        for {
            ...
            go func(c net.Conn) {
                ...
                for {
                    ...
                    if err != nil {
                        if err != io.EOF { log.Printf("conn read error: %v", err) }
                        return
                    }
                }
            }(conn)
...
func main() {
    ...
    if wStr := os.Getenv("WORKERS"); wStr != "" {
        if n, err := strconv.Atoi(wStr); err == nil && n > 0 { workers = n }
    }
